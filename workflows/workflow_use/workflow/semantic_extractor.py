...
class SemanticExtractor:
    ...
    async def extract_interactive_elements(self, browser: BrowserSession) -> List[Dict]:
        ...
        try:
            # Execute JavaScript using browser-use CDP client
            cdp_session = await browser.get_or_create_cdp_session()
            js_expression = f"({js_code})({debug_mode})"
            result_response = await cdp_session.cdp_client.send.Runtime.evaluate(
                params={'expression': js_expression, 'returnByValue': True}, 
                session_id=cdp_session.session_id
            )
            result = result_response.get('result', {}).get('value')
    ...
    async def extract_semantic_mapping(self, browser: BrowserSession) -> Dict[str, Dict]:
        ...
        elements = await self.extract_interactive_elements(browser)
